...
## What you'll learn
- How the Flame Sensor Module detects infrared (IR) light from flames
...
The **Flame Sensor Module** from your Elegoo kit has a special photodiode — a tiny component that is extra-sensitive to **infrared (IR) light**. Ordinary visible light (the light you can see) doesn't affect it much. But flames and very hot objects give off lots of infrared light, which is invisible to human eyes but not to this sensor! When IR light hits the photodiode, it creates a small electrical current, which the module's circuit turns into a voltage you can measure.

The module has two outputs. The **analog output (A0)** gives a number from 0 to 4095. Here is the clever twist: more IR light means *lower* voltage, so more flame = *lower* number! A number close to 4095 means no flame detected (almost no IR light). A very low number, like 100 or 200, means there's lots of IR — something hot and bright is very close! The **digital output (DO)** is simpler: it goes LOW when the IR level crosses a threshold that you set using the small potentiometer on the module. No flame: DO is HIGH. Flame detected: DO is LOW. Notice this is the opposite of what you might expect — LOW means danger!
...
> **Tip:** The DO pin goes LOW when a flame is detected and HIGH when it is safe. This is called *active-low* logic and is common in electronics. Always check the datasheet (or lesson notes!) to know which way a sensor works.

> **Tip:** The potentiometer on the module adjusts the DO threshold. Turn it clockwise to make the sensor LESS sensitive (needs a very intense IR source). Turn it counter-clockwise to make it MORE sensitive (triggers from further away).
...
1. **Active-low logic with `!gpio_get(FLAME_DO_PIN)`** — The flame sensor's DO pin is HIGH when safe and LOW when a flame is detected. So instead of `gpio_get() == 1` meaning "yes," we flip it with `!` (the NOT operator). Now `flame_digital` is `true` when there IS a flame — which feels much more natural to read in the code. Active-low sensors are everywhere in electronics, so get used to this trick!

2. **`adc_read()` returning a lower number for more flame** — This trips up a lot of people! The sensor's analog output voltage drops when it detects more IR. The ADC converts lower voltage to a lower number, so low = strong IR = close flame. That's why `VERY_CLOSE_THRESHOLD` is a small number (500) and the "safe" range is the high numbers near 4095.

3. **`get_proximity_label()`** — This function takes a reading and returns a piece of text (a `const char*`) describing what it means. It uses the same threshold logic as the LED decisions, but converts it to words. Separating the "what does this number mean?" logic into its own function keeps the main loop clean and easy to read.

4. **`sound_alarm(int beeps)`** — The alarm function runs a `for` loop, toggling the buzzer and LED on and off rapidly. Each iteration: buzzer ON + LED red for 80 ms, then both OFF for 60 ms. This creates the classic fast-beep alarm sound. The `sleep_ms()` calls inside the loop are what control the speed — shorter sleeps = faster and more urgent beeping!

5. **`gpio_pull_up(FLAME_DO_PIN)`** — This enables the Pico's built-in pull-up resistor on the DO pin, ensuring the pin reads HIGH (safe) when nothing is connected or when the sensor is idle. It's a safety net to prevent false alarms from a floating pin.

6. **`printf()` with `%-25s`** — The `-25` in the format specifier means "left-align this string in a field of 25 characters." This pads short strings with spaces on the right so that all the columns in your serial output stay neatly aligned, like a table. A small formatting trick that makes your output much easier to read!

7. **Yellow LED for the middle case** — If the analog reading is low (showing significant IR) but DO hasn't triggered yet, the code shows yellow instead of green. This is like a "hmm, something is suspicious" warning state. It could mean the potentiometer threshold is set too high, or it could be a non-flame IR source like a TV remote. Real alarm systems use these multi-level alerts all the time.
...
3. **Threshold tuning:** Use a small screwdriver to slowly adjust the potentiometer on the Flame Sensor Module while pointing a TV remote at it. Find the setting where a remote from 30 cm triggers the DO alarm, but room lighting alone doesn't. Note how the potentiometer setting changes the DO trigger without affecting the analog reading.

4. **Alarm pattern:** Modify the `sound_alarm()` function to make a different pattern. Try a two-tone pattern: beep fast three times, pause 200 ms, then beep fast three times again. Or try an escalating pattern where the beeps get shorter and shorter each time. How urgent can you make it sound?
...
This means the alarm triggers if either sensor detects a problem — just like a real fire alarm! Use `time_us_64()` for the slow-beep timer (from Lesson 7) so the loop doesn't block while waiting between beeps. Print the current state, temperature, humidity, and analog flame reading to the serial monitor every second.

## Summary

The Flame Sensor Module detects infrared light with an analog output (lower number = stronger IR = closer source) and a threshold digital output (LOW = flame detected — active-low logic). By reading both outputs, you built a multi-level alarm: the analog value tells you how close the IR source is, while the digital output triggers the actual alarm. Combining this with an active buzzer and an RGB LED gave you a proper fire alarm system with visual and audio alerts — a real-world project that uses everything you've learned about ADC, GPIO, and program logic!
