...
So far you have only sent signals *out* from the Pico. Now it is time to receive signals *in*! A button is the simplest input device in the world — press it and it makes a connection, release it and the connection breaks. It is like a light switch, but smaller and springier.

The Elegoo Button Switch Module makes things really easy. It has three pins labeled **S** (Signal), **VCC** (power), and **GND** (ground), and it has a **built-in resistor** that keeps the signal pin stable when the button is not pressed. When the button is NOT pressed, the signal pin sits quietly at LOW (0V). When you press the button, it jumps to HIGH (3.3V). That is simple and easy to understand — HIGH means pressed, LOW means not pressed.

Here is a sneaky problem you need to know about: **button bounce**. When you press a button, the metal contacts inside do not make one clean connection. They actually bounce against each other several times in the first few milliseconds, like a ball bouncing before it settles still. The Pico is so fast it reads those bounces as dozens of separate presses! Imagine pressing a doorbell once and hearing it ring 50 times — that is what bounce feels like to your code.

The fix is called **software debouncing**. When you first detect the button going HIGH, wait about 20 milliseconds (not long at all!) and then read it again. If it is still HIGH after that short pause, the bouncing has settled and it is a real press. If the pin changed back to LOW, it was just noise — ignore it. Simple but very effective!
...
The module has only three pins and they are usually labeled right on the board. Signal to GP14, VCC to 3V3, GND to GND. That is it — no extra parts needed. The module handles it all!

The onboard LED will be our output, so no extra LED wiring needed for the basic version.
...
1. **`gpio_set_dir(BUTTON_PIN, GPIO_IN)`** — This tells the Pico the button pin is an INPUT (receiving signals), not an output. You are listening, not talking.
...
3. **Rising edge detection** — We compare `last_button` (what it was before) with `current_button` (what it is now). `!last_button && current_button` means "it was LOW and now it is HIGH" — the exact moment of a fresh press.
...
5. **`led_state = !led_state`** — The `!` flips a true/false value. If `led_state` was `true` (LED on), it becomes `false` (LED off). If it was `false`, it becomes `true`. This is how we toggle!
6. **`last_button = current_button`** — We save what the button was doing this loop so we can detect a change next loop. Without this, we could not spot the rising edge.

## 🚀 Try it

1. **Count your presses** — Add a variable called `press_count` and add 1 to it every time the button is pressed. Then blink the LED that many times in a row. (You will need a second `while` loop inside the button-press block!)
2. **Hold vs tap** — If the button is held for more than 1 second, make the LED blink fast. If it is just tapped quickly, just toggle it once.
